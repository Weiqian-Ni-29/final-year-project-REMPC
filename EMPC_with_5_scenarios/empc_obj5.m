function [fval] = empc_obj5(s)
    % f = x1(t)^2+x2(t)^2
    global N w1 w2 w3 w4 w5;
    nx = 2;
    ...
    %% scenario 5
    Xpred5 = s(4*(nx*(N+1)+N)+nx+1:4*(nx*(N+1)+N)+nx*(N+1)); % predicted states for the fifth scenario
    fval5 = 0;
    for i = 0:N-1
        f = Xpred5(nx*i+1)^4-4*Xpred5(nx*i+1)^3+Xpred5(nx*i+1)^2 +Xpred5(nx*i+nx)^2;
        fval5 = fval5 + f;
    end
    
    %% compute the overall cost as a linear combination of both
    fval = w1*fval1+w2*fval2+w3*fval3+w4*fval4+w5*fval5;
end